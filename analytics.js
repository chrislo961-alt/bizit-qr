...
(function(){
  ...
  document.addEventListener('click',function(e){
    ...
    if(href.includes('/pdf-to-word/'))window.sbkTrack('open_pdf_tool');
    if(href.includes('/qr-code-generator/'))window.sbkTrack('open_qr_tool');
    if(href.includes('/invoice-generator/'))window.sbkTrack('open_invoice_tool');
    if(href.includes('/business-name-generator/'))window.sbkTrack('open_name_generator');
    if(href.includes('/paycheck-calculator/'))window.sbkTrack('open_paycheck_calculator');
    if(href.includes('/hourly-rate-calculator/'))window.sbkTrack('open_rate_calculator');
    if(href.includes('/profit-margin-calculator/'))window.sbkTrack('open_profit_calculator');
    if(href.includes('/break-even-calculator/'))window.sbkTrack('open_break_even_calculator');
    if(href.includes('/tools/'))window.sbkTrack('open_tools_directory');
    const action=classifyAction(el);
    if(action)window.sbkTrack(action);
  });
  ...
  document.addEventListener('input',function(e){
    ...
    if(el&&el.id==='toolSearch'&&!state.searchTracked&&String(el.value||'').length>1){
      ...
      window.sbkTrack('tool_search_used');
    }
  });

  document.addEventListener('change',function(e){
    const el=e.target;
    if(el&&el.type==='file'&&el.files){
      window.sbkTrack('file_selected',{file_count:Math.min(el.files.length,20)});
    }
  });

  function showDependencyError(){
    if(state.errorShown||document.getElementById('sbk-quality-error'))return;
  ...
  function resourceUrl(target){
    return String((target&&target.src)||(target&&target.href)||'');
  }
  ...
  window.addEventListener('error',function(e){
    ...
    const url=resourceUrl(t);
    if(isRequiredToolDependency(url))showDependencyError();
    let host='external';
    try{host=new URL(url,location.href).hostname}catch(_){}
    window.sbkTrack('dependency_load_error',{resource_host:host,required_tool_dependency:isRequiredToolDependency(url)});
  },true);
